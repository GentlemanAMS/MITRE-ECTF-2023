...
def pair(unpaired_fob_bridge, paired_fob_bridge, pair_pin):
    ...
    try:
        pair_success_1 = paired_sock.recv(1)
        while pair_success_1 != b"P":
            pair_success_1 = paired_sock.recv(1)

        pair_success_2 = unpaired_sock.recv(6)
        while len(pair_success_2) != 6:
            pair_success_2 += unpaired_sock.recv(6 - len(pair_success_2))

        print(pair_success_2)
    except socket.timeout:
