 ...
 */
uint32_t receive_board_message(MESSAGE_PACKET *message, uint16_t len) {
  ...
  for (int i = 0; i < message->message_len && i<len; i++) { // check `i` with `len` nevertheless as an attacker can simply modify the length to overflow the message buffer
 ...
 */
uint32_t receive_board_message_by_type(MESSAGE_PACKET *message, uint8_t type, uint16_t len) {
  do {
    receive_board_message(message, len);
