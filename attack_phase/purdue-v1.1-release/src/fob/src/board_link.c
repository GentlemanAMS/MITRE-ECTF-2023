...
void setup_comms(void) {
  ...
  SysCtlPeripheralEnable(SYSCTL_PERIPH_UART1); // UART 1 for board interface
  SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB); // UART 1 is on GPIO Port B
...
/**
 * @brief Receive all characters in the UART FIFO
 *
 * @param uart_p The UART peripheral to use
 *
 * @note This function will not block, it will return immediately
 * It's purpose is to clear the UART FIFO, which is sometimes populated by noise
 *
 */
void recv_all(uint32_t uart_p) {
  while (UARTCharsAvail(uart_p)) {
    UARTCharGet(uart_p);
  }
}
