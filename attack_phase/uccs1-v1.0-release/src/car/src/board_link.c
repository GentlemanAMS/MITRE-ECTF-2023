...
uint8_t* send_board_message(uint8_t *buf, uint8_t bufsz, uint8_t type, int xor, uint8_t *nonc) {
  ...
  uint32_t n = timer_value*22; // TODO timer_value*22;
  ...
  // concate mode
     ...
     // get first nonce
     uint32_t nonce1;
     nonce1 = (nonc[3]<<24)|(nonc[2]<<16)|(nonc[1]<<8)|nonc[0];
     ...
     // testing TODO remove
     char a3[sizeof(uint32_t)*8+1];
     utoa(concat_nonce,a3,10);
     uart_write(HOST_UART, a3, sizeof(a3));
     
     //nonce1 += n;
     
     payload.nonce[3] = (concat_nonce >> 24) & 0xFF;
     payload.nonce[2] = (concat_nonce >> 16) & 0xFF;
     payload.nonce[1] = (concat_nonce >> 8) & 0xFF;
     payload.nonce[0] = concat_nonce & 0xFF;
    }
    */
